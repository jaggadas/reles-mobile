...
import { apiGetSavedRecipe } from "./api";
...
export async function removeRecipeFromGroceryList(videoId: string): Promise<void> {
  const existing = await getGroceryList();
...
export async function isRecipeInGroceryList(videoId: string): Promise<boolean> {
  const list = await getGroceryList();
  return list.some((item) =>
    item.sources.some((s) => s.recipeId === videoId)
  );
}
...
/** Clear all user-specific local data (grocery list). Call on logout. */
